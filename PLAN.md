...
- **Done check-mark:** a green ✓ pins to the right of the Feature cell for `done` rows; it yields to the drag handle (⠿) on hover. The whole Feature cell also gets a **light-green tinge** (`.g-done-row .g-name`, `#E8F7EF`) so completed features read at a glance.
